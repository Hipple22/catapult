...
resize_image.save('./background.png')
...
if __name__=="__main__":
    ...
    while run:
        ...
        if stone.state==STONE_FLY:
            t+=0.5
            stone.move(t,(screen.get_width(),screen.get_height()),decrement_stones)
        ...
        if alien.alive():
            ...
            if collided:
                ...
                explosion.rect.y=(alien.rect.y+alien.rect.height/2)-explosion.rect.height/2
                #crash_sound.play()
        ...
        if game_state==GAME_INIT:
            ...
            sub_title_pos=(screen.get_width()/2 -sub_title_size[0]/2, 100)
            ...
            pos2=(pos1[0]+math.cos(r)*line_len,pos1[1]-math.sin(r)*line_len)
            draw.line(screen,Color(255,0,0),pos1,pos2)

            # 파워와 각도를 텍스트로 표현.
            sf=pygame.font.SysFont("Arial",15)
            text=sf.render("{0} º, {1} m/s".format(direction,int(power)),True,(0,0,0))
            screen.blit(text,pos2)

            # 돌의 개수를 표시
            sf=pygame.font.SysFont("Monospace",20)
            text=sf.render('Stones : {0}'.format(stone_count),True,(0,0,255))
            ...
            # 게임 클리어
            sf=pygame.font.SysFont("Arial",20,bold=True)
            title_str="Congraduations! Mission Complete"
